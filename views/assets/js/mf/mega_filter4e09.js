...
//				});
	...
	location: function(){
		...
		var parts = url.split('http://demo2.ninethemes.net/'),
			last_part = parts.pop();
		...
		return parts.join('http://demo2.ninethemes.net/') + ( last_was_empty || self._options.seo.addSlashAtTheEnd ? '/' : '' ) + query;
	},
	...
	removeMfpFromUrl: function( url ) {
		...
		if( url.match( reg ) ) {
			...
			if( url.match( reg2 ) ) {
				...
				url = url.replace( new RegExp("/" + self.reg_exp_quote(self._options.seo.separator) + "/([a-z0-9\\-_]+,[^\\/\\?]+\\/?)+"), 'http://demo2.ninethemes.net/' );
			}
		}
	...
	getToPost: function( url ) {
		...
		if( self._options.seo.enabled && self._options.seo.usePostAjaxRequests ) {
			...
			if( self._options.seo.addSlashAtTheEnd && url.indexOf('index-2.html') < 0 && ! /\/$/.test( url ) ) {
	...
	createUrl: function( url, attribs, force, ajax ) {
		if( typeof ajax == 'undefined' && typeof url != 'undefined' && url.indexOf('indexd41d.html?') > -1 ) {
			ajax = true;
		}
		...
		if( self._options.seo.enabled && self._options.seo.addSlashAtTheEnd ) {
			...
			if( u.path != '/index.php' && ! /\/$/.test( u.path ) ) {
				u.path += 'http://demo2.ninethemes.net/';
			}
